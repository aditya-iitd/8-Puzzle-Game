#!/usr/bin/env python3
"""
8-Puzzle Format Checker and Solution Validator

This module provides functionality to validate 8-puzzle solutions by:
1. Checking that the reported path length matches the actual moves
2. Verifying that each move in the solution is valid
3. Ensuring the solution leads from start state to end state
4. Validating that the reported cost matches the actual movement costs

Usage:
    python formatChecker.py input_file output_file
"""
...
def build_random_grid():
    """
    Creates a random 8-puzzle configuration.
...
def random_moves(moves):
    """
    Generates a random puzzle by performing specified number of random moves.
    
    Args:
        moves (int): Number of random moves to perform
        
    Prints the resulting start state, end state, and move sequence.
    """
...
# Configuration: Switch between format checking and random move generation
MODE = "FORMAT_CHECKER"
# MODE = "RANDOM_MOVES"
...
if __name__ == '__main__':
    ...
    Command line usage:
    - Format checking mode: python formatChecker.py input_file output_file
    - Random moves mode: python formatChecker.py number_of_moves
    """
